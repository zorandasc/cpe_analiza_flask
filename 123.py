...
def get_cpe_inventory_sub_city(major_city_id):
    params = {"major_city_id": major_city_id}
    ...
    SQL_QUERY = """
        SELECT
            ...
            ci.updated_at
        FROM cities c
        LEFT JOIN cpe_inventory ci ON c.id = ci.city_id
        LEFT JOIN cpe_types ct ON ct.id = ci.cpe_type_id
        -- GET THE MAJOR CITY AND ALSO HIS SUBCITIES
...
def get_cpe_inventory_subcities(schema_list: list, week_end: datetime.date):
    if not schema_list:
        # Return empty data lists immediately if no active CPE types are found
        return []
